      ...
      sections: [
        {
          label: '서비스 소개',
          content:
            '모임 장소 선정 → 투표 → (예약 관리) → 비용 정산을 하나의 서비스에서 처리하는 그룹 모임 운영 플랫폼입니다.',
        },
        {
          label: '차별점',
          content:
            '모임 하나를 위해 카카오톡(날짜 논의) → 네이버 지도(식당 탐색) → 캐치테이블(예약) → 카카오톡(정산 요청) → 계산기(더치페이)까지 여러 앱을 오가야 하며, 장소 선정과 정산에서 특히 시간이 많이 듭니다. 얌피(yummpi)는 이 과정을 한 흐름으로 묶습니다.',
        },
      ],
    },
    techStack: [
